...
function buildItems(pool: string[], seg: number) {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, text: '' }));
  }
  if (pool.length > totalSlots) {
    console.warn(
      `[DomeGallery] Provided item count (${pool.length}) exceeds available tiles (${totalSlots}). Some items will not be shown.`
    );
  }

  const usedItems = Array.from({ length: totalSlots }, (_, i) => pool[i % pool.length]);

  // Try to avoid adjacencies (basic shuffle)
  for (let i = 1; i < usedItems.length; i++) {
    if (usedItems[i] === usedItems[i - 1]) {
      for (let j = i + 1; j < usedItems.length; j++) {
        if (usedItems[j] !== usedItems[i]) {
          const tmp = usedItems[i];
          usedItems[i] = usedItems[j];
          usedItems[j] = tmp;
          break;
        }
      }
...
export default function DomeGallery({
  ...
  maxRadius = Infinity,
  padFactor = 0.25,
  overlayBlurColor = '#0a0a0b', // Match site background
  ...
  const generatedItems = useMemo(() => buildItems(items, segments), [items, segments]);
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const originalContent = overlay.firstElementChild;
      if (originalContent) {
        const cloned = originalContent.cloneNode(true) as HTMLElement;
        animatingOverlay.appendChild(cloned);
      }
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      if (contentSpan) {
        ...
        clonedText.textContent = contentSpan.textContent;
        // Text is much bigger in the enlarged view
        clonedText.className = 'font-mono text-3xl md:text-4xl tracking-wider text-accent pointer-events-none select-none text-center px-6';
        overlay.appendChild(clonedText);
      }
            ...
            {generatedItems.map((it, i) => (
              ...
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.text || 'Open item'}
                  onClick={onTileClick}
                  onPointerUp={onTilePointerUp}
                >
                  <span className="font-mono text-xs md:text-sm tracking-wider text-accent select-none px-2 text-center text-balance">
                    {it.text}
                  </span>
                </div>
              </div>
